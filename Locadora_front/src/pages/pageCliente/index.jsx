...
// (Sem alterações neste componente, ele é apenas para ADICIONAR novos)
...
    cpf: '',
    dtNascimento: '',
    estaAtivo: true // --- ALTERAÇÃO: Adicionado estado para o status
  });
  
...
      cpf: '', 
      dtNascimento: '',
      estaAtivo: true // --- ALTERAÇÃO: Resetar o estado
    });
    setIsEditing(false);
    setCurrentCliente(null);
  }

  // --- ALTERAÇÃO: Atualizada para lidar com boolean vindo do select ---
...
    if (type === 'checkbox') {
      finalValue = checked;
    } else if (name === 'estaAtivo') {
      finalValue = value === 'true'; // Converte a string "true" ou "false" para boolean
    } else {
      finalValue = value;
    }
    
...
    } else if (isEditing && !currentCliente?.cpf) {
      // Validação para dependente (nome, sexo, dtNasc)
      // O campo estaAtivo sempre terá um valor (true/false), então não precisa de !formData.estaAtivo
...
        } else {
          // --- ALTERAÇÃO: Enviar o campo estaAtivo na atualização do dependente ---
          await api.put(`/Cliente/dependentes/${currentCliente.numInscricao}`, {
            ...currentCliente,
            nome: formData.nome,
            sexo: formData.sexo,
            dtNascimento: formData.dtNascimento,
            estaAtivo: formData.estaAtivo // Enviando o novo status
          });
...
  // --- ALTERAÇÃO: Popular o estaAtivo ao editar ---
...
      estaAtivo: cliente.estaAtivo // Popula o status atual (seja sócio ou dependente)
    });
    
    window.scrollTo(0, 0);
  }
...
        {/* --- ALTERAÇÃO: Dropdown de Status para editar dependente --- */}
        {isEditing && !currentCliente?.cpf && (
          <>
            <label>Status:</label>
            <select 
              name="estaAtivo" 
              value={formData.estaAtivo} 
...
t             <p>Status: <span>{cliente.estaAtivo ? "Ativo" : "Inativo"}</span></p>
                {isSocio && (
